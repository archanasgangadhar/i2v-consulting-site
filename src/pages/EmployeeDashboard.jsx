...
export default function EmployeeDashboard() {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      window.removeEventListener("i2v:auth", decodeToken);
    };
  }, []);
  ...
  // Compute a friendly username from token (prefer sub, then email). If it's an email, show the local-part before '@'.
  const rawUsername = state.decoded?.sub || state.decoded?.email || '—';
  const username = rawUsername && rawUsername.includes("@") ? rawUsername.split("@")[0] : rawUsername;
    ...
    <div className="max-w-7xl mx-auto px-6 py-10">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-600">Overview of your account and session</p>
        </div>
      ...
      <fieldset className="mt-6 bg-white p-4 shadow rounded">
        <legend className="text-sm font-semibold mb-2">
          <h1 className="text-2xl font-bold">Onboard Details</h1>
        </legend>
        <Hr />
